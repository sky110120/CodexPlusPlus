...
fn injection_script_menu_exposes_stepwise_switch_and_syncs_panel() {
    ...
    assert!(script.contains("data-codex-plus-setting=\"stepwise\""));
    assert!(script.contains("data-codex-plus-setting=\"answerOutline\""));
...
fn stepwise_runtime_stops_work_when_disabled() {
    ...
    assert!(script.contains("function clearSubmitRetryTimers()"));
    assert!(script.contains("function scheduleSubmitRetry(callback, delay)"));
    assert!(script.contains("bridgeRequestSequence: 0"));
    ...
    assert!(script.contains("if (!isCurrentRuntime() || !stepwiseEnabled()) return false;"));
    assert!(script.contains("state.stepwiseEpoch += 1"));
...
const codexPlusStatsigModelOriginalValues = new Map();
const codexPlusModelPatchSnapshot = () => null;
{function_source}
