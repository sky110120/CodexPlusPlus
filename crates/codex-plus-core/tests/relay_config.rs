...
fn write_live_mcp_context_fixture(home: &std::path::Path) {
    std::fs::write(
        home.join("config.toml"),
        r#"model = "old"

[mcp_servers.managed]
command = "live-managed"
...
fn disabled_managed_mcp_context() -> &'static str {
    r#"[mcp_servers.managed]
enabled = false
command = "disabled-managed"
...
#[test]
fn apply_relay_files_with_context_does_not_restore_disabled_live_mcp() {
    let temp = tempfile::tempdir().unwrap();
    write_live_mcp_context_fixture(temp.path());
...
#[test]
fn apply_relay_profile_with_context_does_not_restore_disabled_live_mcp() {
    let temp = tempfile::tempdir().unwrap();
    write_live_mcp_context_fixture(temp.path());
    let profile = RelayProfile {
        id: "relay-mcp-policy".to_string(),
        relay_mode: RelayMode::PureApi,
