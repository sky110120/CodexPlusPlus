...
fn delete_cleanup_and_undo_restores_session_index_and_global_state() {
    ...
    let restored_state: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(&global_state).unwrap()).unwrap();
    let restored_ids = restored_state["projectless-thread-ids"].as_array().unwrap();
    assert!(restored_ids.iter().any(|value| value == "t1"));
    assert!(restored_ids.iter().any(|value| value == "other"));
    assert_eq!(
        restored_state["thread-projectless-output-directories"]["t1"],
        "/tmp/t1"
    );
    assert_eq!(
...
fn undo_fails_before_restoring_database_when_global_state_changed_after_delete() {
    ...
    let restored_state: serde_json::Value =
        serde_json::from_str(&fs::read_to_string(&global_state).unwrap()).unwrap();
    assert_eq!(restored_state["changed-after-delete"], true);
    let restored_ids = restored_state["projectless-thread-ids"].as_array().unwrap();
    assert!(restored_ids.iter().any(|value| value == "t1"));
    assert!(restored_ids.iter().any(|value| value == "other"));
}
