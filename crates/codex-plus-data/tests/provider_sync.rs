use codex_plus_data::{
    ...
    SessionIndexCleanupReason, apply_session_index_cleanup,
...
fn provider_sync_reports_rollback_when_rewrite_write_fails() {
    ...
    let blocked_rollout = home.join("sessions/rollout-z.jsonl");
    ...
    let result = run_provider_sync_with_target_and_progress(Some(&home), None, |event| {
        let start_rewriting = event.phase == ProviderSyncProgressPhase::Rewriting
            && event.applied_rewrite_files == 0;
        progress.push(event);
        if start_rewriting && !injected_failure {
            fs::remove_file(&blocked_rollout).unwrap();
            fs::create_dir(&blocked_rollout).unwrap();
...
#[test]
fn provider_sync_excludes_subagents_from_provider_updates_and_catalog() {
    let tmp = tempdir().unwrap();
    let home = tmp.path().join(".codex");
    let sqlite_dir = home.join("sqlite");
    fs::create_dir_all(&sqlite_dir).unwrap();
    ...
    for (thread_id, rollout_path) in [
        ...
        let thread_source = if thread_id == "child-existing" {
            Some("subagent")
        } else {
            None
        };
    ...
    let existing_child_count: i64 = catalog
        .query_row(
            "SELECT COUNT(*) FROM local_thread_catalog WHERE thread_id = 'child-existing'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(existing_child_count, 0);
...
#[test]
fn provider_sync_preserves_explicit_user_threads_over_subagent_fallbacks() {
    let tmp = tempdir().unwrap();
    let home = tmp.path().join(".codex");
    let sqlite_dir = home.join("sqlite");
    fs::create_dir_all(&sqlite_dir).unwrap();
    ...
    db.execute(
        "INSERT INTO threads VALUES (
            'explicit-user', 'openai', 0, 0, 'C:/old', 'Explicit user', ?1,
            '{\"subagent\":{\"other\":\"review\"}}', 100000, 200000, 'User', 'main'
        )",
    ...
    .unwrap();
    db.execute(
        "INSERT INTO thread_spawn_edges VALUES ('parent-thread', 'explicit-user')",
        [],
    )
    .unwrap();
    drop(db);
    ...
    let row: (String, i64, String) = db
        .query_row(
            "SELECT model_provider, has_user_event, cwd FROM threads WHERE id = 'explicit-user'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)),
        )
        .unwrap();
    ...
    let catalog_provider: String = Connection::open(&catalog_db)
        .unwrap()
        .query_row(
            "SELECT model_provider FROM local_thread_catalog WHERE thread_id = 'explicit-user'",
            [],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(catalog_provider, "apigather");
}
...
fn provider_sync_prunes_archived_and_ineligible_catalog_rows() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            ("local".to_string(), "guardian".to_string()),
            ("local".to_string(), "orphan".to_string()),
